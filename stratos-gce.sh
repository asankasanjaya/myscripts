...
# to you under the Apache License, Version 2.0 (theST
...
#export STRATOS_SRC_REPO=https://github.com/suriyapriya/stratos
#export STRATOS_SRC_VERSION="gce-4.0.0"
...
function puppet_base_setup() {
  ...
  [ -d /etc/puppet/modules/python_agent/files ] || sudo mkdir -p /etc/puppet/modules/python_agent/files
...
sudo cp -f $STRATOS_SOURCE_PATH/products/python-cartridge-agent/distribution/target/apache-stratos-python-cartridge-agent-*.zip /etc/puppet/modules/python_agent/files
...
function installer() {
  ...
  openstack_identity="username"
  openstack_credential="password"
  openstack_jclouds_endpoint="openstack_endpoint"
  ...
  sed -i "s:^export gce_provider_enabled=.*:export gce_provider_enabled='$gce_provider_enabled':g" $CFG_FILE
  sed -i "s:^export gce_identity=.*:export gce_identity='$gce_identity':g" $CFG_FILE
  sed -i "s:^export gce_credential=.*:export gce_credential='${gce_credential//$'\n'/\n}':g" $CFG_FILE
  sed -i "s:^export gce_projectname=.*:export gce_projectname='$gce_projectname':g" $CFG_FILE
...
function checkout() {
  ...
  git clone $STRATOS_SRC_REPO $STRATOS_SOURCE_PATH
